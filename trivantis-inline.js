...
function ObjInlineBuild() {
  ...
  divStart += '><a name="'+this.name+'anc" tabindex="' + this.z + '"'
...
function ObjInlineCreatePlayer()
{
	if(is.YTScriptLoaded)
	{
		...
		{
			this.YTPlayer = trivPlayer.createMediaElemPlayer(null, null, this);
		}
		else
		{
...
function ObjInlineRefresh(){
	...
	if(iFrame)
	{
		...
		if(this.Score) this.Score.set('0');
	}
}
...
function ObjInlineSetupWinEvents(varBTScore , varBTPath, webWinObj){
	if(!is.ie8)
	{
		getDisplayWindow().addEventListener('message', function(event) {
			...
			if (messagedatatype=='branchtrack:player:scene' && data.details.scene.score > 0) {
					varBTScore.set(data.details.scene.score)
			}
