...
p.updateFillSize = function ( pictureId ){
	if( is.svg && this.str_SvgMapPath )
	{
		...
		else
		{
			...
			if(pictureId.indexOf("_over") >= 0)
				state = "Over";
			else if(pictureId.indexOf("_down") >= 0)
				state = "Down";
			else if(pictureId.indexOf("_disabled") >= 0)
				state = "Disabled"
			else
				state = "Normal"
...
function ObjButtonSetDisable( bSet )
{
	...
	if(!is.svg || !this.bUseSvgFile)
	{
		if (this.imgDisabledSrc && bSet) 
			this.change(this.imgDisabledSrc, false, null, this.imgDisabledOpacity)
		else if (!this.imgDisabledSrc && bSet) 
		{
			this.change(this.imgOffSrc, false, null, this.imgOffOpacity);
		}
	...
	else
	{
		if (this.str_SvgStyleDisabled_Url && bSet) 
			this.change(this.str_SvgStyleDisabled_Url, false , this.str_TxtFill_Disabled, this.imgDisabledOpacity)
		else if (!this.str_SvgStyleDisabled_Url && bSet) 
		{
			this.change(this.str_SvgStyleNormal_Url, false , this.str_TxtFill, this.imgOffOpacity);
		}
...
function ObjButtonBuild() {
  ...
  this.divInt = '<button type="button" name="'+this.name+'btn" id="'+this.name+'btn" tabindex="' + this.z +'"'
 ...
 if(this.hasAct && !this.bHasClickMap)
  ...
  this.divInt += '>'
  if((!this.hasOuterShadow || is.ie9 ) && (!is.svg || !this.bUseSvgFile))
  {
...
{	
	...
	txtReturn += "<pattern id=\"Text_" + this.fuID + "\" x=\"0\" y=\"0\" width=\"" + this.w + "\" height=\""+ this.h +  "\" patternUnits=\"userSpaceOnUse\">\n";
	txtReturn += "<image xlink:href=\""+ this.td + "\" x=\"0\" y=\"0\" width=\"" + this.w + "\" height=\""+this.h +  "\" preserveAspectRatio=\"none\">\n";
	txtReturn += "</pattern>\n";
	
	txtReturn += "<pattern id=\"Text_" + this.fuID + "_over\" x=\"0\" y=\"0\" width=\"" + this.w + "\" height=\""+ this.h +  "\" patternUnits=\"userSpaceOnUse\">\n";
	txtReturn += "<image xlink:href=\""+ this.tdO + "\" x=\"0\" y=\"0\" width=\"" + this.w + "\" height=\""+this.h +  "\" preserveAspectRatio=\"none\">\n";
	txtReturn += "</pattern>\n";
	
	txtReturn += "<pattern id=\"Text_" + this.fuID + "_down\" x=\"0\" y=\"0\" width=\"" + this.w + "\" height=\""+ this.h +  "\" patternUnits=\"userSpaceOnUse\">\n";
	txtReturn += "<image xlink:href=\""+ this.tdD + "\" x=\"0\" y=\"0\" width=\"" + this.w + "\" height=\""+this.h +  "\" preserveAspectRatio=\"none\">\n";
	txtReturn += "</pattern>\n";
	
	txtReturn += "<pattern id=\"Text_" + this.fuID + "_disabled\" x=\"0\" y=\"0\" width=\"" + this.w + "\" height=\""+ this.h +  "\" patternUnits=\"userSpaceOnUse\">\n";
	txtReturn += "<image xlink:href=\""+ this.tdDi + "\" x=\"0\" y=\"0\" width=\"" + this.w + "\" height=\""+this.h +  "\" preserveAspectRatio=\"none\">\n";
	txtReturn += "</pattern>\n";
...
function ObjButtonDown(e) {
  ...
  if(!is.svg || !this.bUseSvgFile)
  {
	  if (this.selected) {
		this.selected = false
		if (this.imgOnSrc) this.change(this.imgOnSrc, false, null, this.imgOnOpacity)
		this.onDeselect()
	  }
	  else {
  ...
  else
  {
	  if (this.selected) {
		this.selected = false
		if (this.str_SvgStyleDown_Url) this.change(this.str_SvgStyleDown_Url, false , this.str_TxtFill_Down, this.imgOnOpacity)
		this.onDeselect()
	  }
	  else {
		if (this.checkbox) this.selected = true
...
function ObjButtonUp(e) {
  ...
  if(!is.svg || !this.bUseSvgFile){
	   if (!this.selected) {
		if (this.imgRollSrc) this.change(this.imgRollSrc, false, null, this.imgRollOpacity)
		else if (this.imgOnSrc) this.change(this.imgOffSrc, false, null, this.imgOnOpacity)
	  }
  }
  else{
	  if (!this.selected) {
		if (this.str_SvgStyleOver_Url) this.change(this.str_SvgStyleOver_Url, false , this.str_TxtFill_Over, this.imgRollOpacity)
		else if (this.str_SvgStyleDown_Url) this.change(this.str_SvgStyleNormal_Url, false, this.str_TxtFill, this.imgOnOpacity)
	  }
  }
...
function ObjButtonOver() {
  ...
  { 
	if(this.imgRollSrc && !this.selected)
		this.change(this.imgRollSrc, false, null, this.imgRollOpacity); 
  }
  else
  {
	  if(this.str_SvgStyleOver_Url && !this.selected) 
		  this.change(this.str_SvgStyleOver_Url, false , this.str_TxtFill_Over, this.imgRollOpacity);
  }
...
function ObjButtonChange(img, bResp , text, stateOpacity) {
  if(typeof(bResp) == "undefined")
		...
		if(is.svg){
			...
			for(var i = 0 ; i < paths.length ; i++){
				...
				if(style){
					...
					if(fill.length > 0 && fill.indexOf("Picture") >= 0)
...
function ObjLoadProps()
{
	if(is.jsonData != null)
	{
		...
		if(obj)
		{
			...
			this.tdDi = typeof(obj.tdDi)!="undefined"?obj.td:this.tdDi;
...
function ObjRespChanges()
{
	...
	if(!this.bOffPage)
	{
		...
		{
		 	if(this.bDisabled)
				this.change(this.imgDisabledSrc, true, null, this.imgDisabledOpacity);
			else
				this.change(this.imgOffSrc, true, null, this.imgOffOpacity);
		}
		else
		{
			if(this.bDisabled)
...
{
	if(this.base64Normal && this.str_SvgStyleNormal_Url.indexOf("nonzero")==-1)
	{
	...
	if(this.base64Over && this.str_SvgStyleOver_Url.indexOf("nonzero")==-1)
	{
	...
	if(this.base64Down && this.str_SvgStyleDown_Url.indexOf("nonzero")==-1)
	{
	...
	if(this.base64Disabled && this.str_SvgStyleDisabled_Url.indexOf("nonzero")==-1)
	{
